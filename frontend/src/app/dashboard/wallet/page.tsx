...
import type { PendingTransaction } from "@/types/ads";
...
import { ROLE_USER, ROLE_ADVERTISER, ROLE_ADMIN, BLOCK_CONFIRMATIONS } from "@/lib/constants";
...
export default function WalletPage() {
  ...
  const [pendingTx, setPendingTx] = useState<PendingTransaction[]>(() => {
    try {
      const saved = localStorage.getItem("pending_deposits");
      return saved ? JSON.parse(saved) : [];
    } catch {
      return [];
    }
  });
  const walletRefetchInterval = pendingTx.length > 0 ? 4000 : false;
  const { data: wallet, isLoading, error } = useWallet(walletRefetchInterval);
  ...
  useEffect(() => {
    localStorage.setItem("pending_deposits", JSON.stringify(pendingTx));
  }, [pendingTx]);
  ...
  const onChainBalance = rawBalance !== undefined ? (Number(rawBalance) / 1e18).toFixed(2) : null;

  useEffect(() => {
    if (pendingTx.length === 0 || !cfg) return;
      ...
      try {
        ...
        const updated = await Promise.all(
          pendingTx.map(async (tx) => {
            if (tx.confirmations >= tx.target_confirmations) return tx;
            try {
        ...
        const stillPending = updated.filter((tx) => {
          if (tx.confirmations >= tx.target_confirmations) {
        ...
        if (JSON.stringify(stillPending) !== JSON.stringify(pendingTx)) {
          setPendingTx(stillPending);
        }
      } catch (err) {
  ...
  }, [pendingTx, cfg, connectWallet, queryClient]);
  ...
  const handleDeposit = async () => {
    ...
    try {
      ...
      const newPending: PendingTransaction = {
        ...
        type: "DEPOSIT",
        confirmations: 0,
        target_confirmations: BLOCK_CONFIRMATIONS,
      };
      setPendingTx((prev) => [newPending, ...prev]);
  ...
  const handleWithdraw = async () => {
    ...
    if (num > Number(wallet?.available ?? 0)) {
      toast.error("Insufficient available balance");
      return;
    }
    try {
      const hash = await withdrawOnChain(withdrawAmount);
      ...
      <div className="space-y-6">
        {isAdvertiser ? (
          <div className="rounded-2xl border border-white/[0.08] bg-white/[0.02] p-6 md:p-8">
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
              <div className="border-b md:border-b-0 md:border-r border-white/10 pb-4 md:pb-0 md:pr-6">
                ...
                <p className="text-3xl font-black text-white">{Number(wallet?.available ?? wallet?.balance ?? 0).toFixed(2)} {symbol}</p>
              ...
              <div className="border-b md:border-b-0 md:border-r border-white/10 pb-4 md:pb-0 md:pr-6">
                ...
                <p className="text-3xl font-black text-white">{Number(wallet?.frozen ?? 0).toFixed(2)} {symbol}</p>
              ...
              <div>
                ...
                <p className="text-3xl font-black text-white">{Number(wallet?.balance ?? 0).toFixed(2)} {symbol}</p>
            ...
            {showDeposit && (
              <div className="mt-4 p-4 rounded-xl border border-white/[0.08] bg-white/[0.03]">
                <div className="flex items-center justify-between mb-2">
                  ...
                  {isConnected && (
                    <span className="text-xs text-[#22D3EE] font-mono">
                      Wallet Balance: {isBalanceFetching && onChainBalance === null ? "Loading..." : `${onChainBalance ?? "0.00"} ${symbol}`}
                    </span>
                  )}
                </div>
                <div className="flex gap-3">
          ...
          <div className="rounded-2xl border border-white/[0.08] bg-white/[0.02] p-6 md:p-8">
            ...
            <div className="mt-4 p-4 rounded-xl border border-white/[0.08] bg-white/[0.03]">
              <div className="flex items-center justify-between mb-2">
                <span className="text-xs text-white/50">Withdraw {symbol}</span>
                <span className="text-xs text-[#6EE7B7] font-mono">
                  Available: {Number(wallet?.available ?? 0).toFixed(2)} {symbol}
                </span>
              </div>
              <div className="flex gap-3">
                <input
