  ...
  if (error) {
    return (
      <div className="rounded-2xl bg-red-900/20 p-6 border border-red-900/50 text-red-300">
        Failed to load statistics. The link might not exist.
      </div>
    );
  }
  ...
  if (!stats || stats.total_clicks === 0) {
    return (
      <div className="rounded-2xl bg-white/[0.02] border border-white/[0.08] p-12 text-center">
        ...
        <p className="text-white/50">Share your link to start seeing statistics here.</p>
      </div>
    );
  }

  const { browsers: browserData, devices: deviceData } = stats.formatted;
        ...
        <div className="h-72 w-full">
          <ResponsiveContainer width="100%" height="100%">
            <BarChart data={stats.clicks_per_day} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
              ...
              <Tooltip
                contentStyle={{ backgroundColor: '#111', borderColor: 'rgba(255,255,255,0.1)', borderRadius: '0.75rem', color: '#fff' }}
            ...
            <ResponsiveContainer width="100%" height="100%">
              <PieChart>
                <Pie
                  ...
                  dataKey="value"
                />
                <Tooltip contentStyle={{ backgroundColor: '#111', borderColor: 'rgba(255,255,255,0.1)', borderRadius: '0.75rem', color: '#fff' }} />
              </PieChart>
            </ResponsiveContainer>
          </div>
