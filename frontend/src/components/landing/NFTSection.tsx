...
export default function NFTSection() {
  ...
  const buttonState = (() => {
    if (isLoading) return { text: "Preparing Mint...", icon: <Loader2 size={16} className="animate-spin" />, disabled: true };
    if (isError) return { text: "Minting Unavailable", icon: <Sparkles size={16} />, disabled: true };
    if (hasNFT) return { text: "Pass Active (Already Owned)", icon: <Check size={16} className="text-[#0A0A0A]" />, disabled: true };
    if (isConnecting) return { text: "Connecting Wallet...", icon: <Loader2 size={16} className="animate-spin" />, disabled: true };
    if (pendingAction === ACTION_APPROVE) return { text: `Approving ${tokenSymbol}...`, icon: <Loader2 size={16} className="animate-spin" />, disabled: true };
    if (pendingAction === ACTION_MINT) return { text: "Minting Pass...", icon: <Loader2 size={16} className="animate-spin" />, disabled: true };
    if (!isConnected) return { text: "Connect & Mint Pass", icon: <Sparkles size={16} />, disabled: false };
    return { text: `Mint Pass (${displayPrice} ${tokenSymbol})`, icon: <Sparkles size={16} />, disabled: isBusy };
  })();

  const baseBtnClasses = "px-8 py-4 rounded-xl text-base font-extrabold tracking-tight inline-flex items-center gap-2 transition-all duration-300";
  ...
  return (
    <section id="nft-pass" className="relative py-24 px-6 md:px-12 max-w-7xl mx-auto overflow-hidden">
