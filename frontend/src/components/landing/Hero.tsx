...
export default function Hero() {
  ...
  return (
    <section className="relative pt-28 sm:pt-36 lg:pt-48 pb-12 sm:pb-20 lg:pb-32 px-4 sm:px-6 md:px-12 max-w-7xl mx-auto">
      ...
      <div className="fade-in inline-flex items-center gap-2 mb-8 px-4 py-2 rounded-full text-xs font-mono-dm border border-[#6EE7B7]/30 text-[#6EE7B7] bg-[#6EE7B7]/5">
        <Zap size={12} />
        <span className="truncate max-w-[200px] sm:max-w-none"><span className="hidden sm:inline">Lightning-fast redirects · </span>Earn crypto · Trusted by thousands</span>
      </div>

      {/* Headline */}
      <h1 className="fade-in text-5xl md:text-7xl lg:text-8xl font-black tracking-tight leading-none mb-4 sm:mb-6 text-glow [animation-delay:0.1s]">
        SHORT LINKS.<br />
      ...
      <div className="fade-in flex flex-col sm:flex-row items-start sm:items-center gap-4 [animation-delay:0.3s]">
        <Link href="/register"className="btn-primary px-4 sm:px-8 py-3.5 sm:py-4 rounded-xl text-sm sm:text-base inline-flex items-center gap-2">
          Start shortening <ArrowRight size={18} />
        </Link>
        <Link href="/register/advertiser"className="px-4 sm:px-8 py-3.5 sm:py-4 rounded-xl text-sm sm:text-base font-bold tracking-tight inline-flex items-center gap-2 bg-[#22D3EE] text-[#0A0A0A] hover:bg-[#67E8F9] hover:-translate-y-0.5 hover:shadow-[0_8px_30px_rgba(34,211,238,0.3)] transition-all duration-200">
          <Megaphone size={18} />
          Advertise with us
        </Link>
      ...
      {/* Demo card */}
      <div className="fade-in mt-20 rounded-2xl p-4 sm:p-6 md:p-8 max-w-xl glow-green bg-white/[0.06] border border-white/[0.12] [animation-delay:0.4s]">
